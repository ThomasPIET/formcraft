...
async function updateForm(formId, formName, questions) {
  try {
    await db.$transaction(async (prisma) => {
      await prisma.question.deleteMany({
        where: { formId },
    ...
    console.error("Erreur lors de la mise à jour dans formServices :", error);
    throw new Error("Erreur dans la fonction formServices");
  }
}

export { saveForm, deleteForm, checkTitle, getForm, updateForm };
